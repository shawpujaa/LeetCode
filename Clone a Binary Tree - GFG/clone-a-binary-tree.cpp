...
class Solution{
  public:
    ...
    void assignRandom(Node *tree, Node *head)
    {
        if(!tree)
        return;
        ...
        if(tree->random)
        {
            auto mt=mp.find(tree->random);
            head->random=mt->second;
        }
        assignRandom(tree->left, head->left);
        assignRandom(tree->right, head->right);
    }
    
    Node* cloneTree(Node* tree)
    {
       //Your code here
       Node *head=clone(tree);
       assignRandom(tree, head);
